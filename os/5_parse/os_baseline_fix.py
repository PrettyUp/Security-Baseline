    ...
    if model == "ip":
        ...
        if ip is None:
            ...
            for ip_addr in ip_list:
                # 如果指定的ip对应的文件并不存在则跳过
                if not os.path.exists(f"../4_report/{ip_addr}_os_report.html"):
                    print(f'sorry, file "../4_report/{ip_addr}_os_report.html" is not exist, it will be skip')
                    continue
                gen_fix_obj = GenOSFixScript(ip_addr)
